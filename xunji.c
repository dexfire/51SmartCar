...
code struct time_config speed_level_configs[] = {
	...
	// 完全跟线走，稍慢, 表现良好
	// 6T 模式下动不了 
	{ 200,	5,5,  1,5,0,5,	5,1,5,0,	12500,12000 },

	// Mode 2
	// 完全根线，小弯大弯表现良好，修复能力强，表现优秀
	{ 200,	6,6,	3,6,2,6, 6,3,6,2,12500,12000 },

	// Mode 3
	// 大弯修复能力弱，走着走着就偏了
	//{6,6,  1,5,0,5, 5,1,5,0,12000,12000 },
	// 尝试减小偏转力度
	{200,6,6,  1,4,0,4, 4,1,4,0,12000,12000 },

	// Mode 4
	// 6T 模式下工作良好，表现很好   12T 也还可以
	{200,9,9,  3,5,0,5, 5,3,5,0,12000,12000 },

	// Mode 5
	// 高速，拐弯溢出边界
	//{15,15,  2,7,1,7, 7,2,7,1,12000,12000 },
	// 尝试再次降低修正幅度   效果还可以   稳定性差
	{300,13,13,  5,7,3,7, 7,5,7,3,12000,12000 },
};
...
void init()
{
	...
	TH1=_iror_(65536-1000,8);                 //12M，定时2ms
	TL1=65536-1000;
	ET1=1;
	TR1=0;
...
void crossroad()
{
	if(XUNJI_1 && XUNJI_2 && XUNJI_3 && XUNJI_4)  //依旧四灯全灭,有理由相信遇到十字路口
	{
		...
		// while(ECHO);
		// TR2=0;             //关闭T2中断
		// if(n>=2) distance=999;  //即没障碍
		// else distance = ((TL2+TH2*256) /1000) *17;  //cm
		// //OLED_ShowNum3(110,2,distance);

		// if(distance < 15)     //小于15cm,到达终点，停止蜂鸣器响
		// 	stop();	
		// else if(distance < 30)   //大于15小于30,左转
		// 	turn_left_90();
		// else if(distance < 50)   //大于30小于50,右转
		// 	turn_right_90();
		// else
		// 	go_straight();
	}
